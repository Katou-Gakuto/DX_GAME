...
// {
// 	return UtilStateConditionFunction::GetNextStateByPlayerDistance(character);
// }
...
// {
// 	// 注意: この条件はState固有のメンバ変数(mstPreAttackTime, mbLeftMoveFlag)に依存しているため
// 	//       完全に外部関数化することはできません。State側で条件を保持する必要があります。
// 	//       推測: 時間経過とHPに基づいた条件判定
...
// 	// 外部関数を使用: プレイヤーの方角を向いているか判定
// 	if (UtilStateConditionFunction::IsFacingPlayer(character))
// 	{
...
// {
// 	// 注意: この条件はState固有のメンバ変数(mstEscapeTime)に依存しているため
// 	//       完全に外部関数化することはできません。State側で条件を保持する必要があります。
// 	//       推測: 逃げ時間経過に基づいた条件判定
