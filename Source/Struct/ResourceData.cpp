...
int DisplaySize::Left_SeparateRatioWidth(float startRatio, float endRatio)
{
    return static_cast<int>((x * endRatio)) - static_cast<int>(x * startRatio);
}

// 上を起点にした離れた割合(0～1)から縦軸の座標を取得
int DisplaySize::Up_SeparateRatioHeight(float startRatio, float endRatio)
{
    return static_cast<int>((y * endRatio)) - static_cast<int>(y * startRatio);
}

// 右を起点にした離れた割合(0～1)から横軸の座標を取得
int DisplaySize::Right_SeparateRatioWidth(float startRatio, float endRatio)
{
    return static_cast<int>((x * (1.0f - startRatio))) - static_cast<int>(x * (1.0f - endRatio));
}

// 下を起点にした離れた割合(0～1)から縦軸の座標を取得
int DisplaySize::Down_SeparateRatioHeight(float startRatio, float endRatio)
{
    return static_cast<int>((y * (1.0f - startRatio))) - static_cast<int>(y * (1.0f - endRatio));
}

// 左上を起点にした離れた割合(0～1)から座標を取得
Vector2_Int DisplaySize::LeftUp_SeparateRatio(Vector2 startRatio, Vector2 endRatio)
{
    Vector2_Int pos;
    pos.x = Left_SeparateRatioWidth(startRatio.x, endRatio.x);
    pos.y = Up_SeparateRatioHeight(startRatio.y, endRatio.y);
    return pos;
}

// 右上を起点にした離れた割合(0～1)から座標を取得
Vector2_Int DisplaySize::RightUp_SeparateRatio(Vector2 startRatio, Vector2 endRatio)
{
    Vector2_Int pos;
    pos.x = Right_SeparateRatioWidth(startRatio.x, endRatio.x);
    pos.y = Up_SeparateRatioHeight(startRatio.y, endRatio.y);
    return pos;
}

// 左下を起点にした離れた割合(0～1)から座標を取得
Vector2_Int DisplaySize::LeftDown_SeparateRatio(Vector2 startRatio, Vector2 endRatio)
{
    Vector2_Int pos;
    pos.x = Left_SeparateRatioWidth(startRatio.x, endRatio.x);
    pos.y = Down_SeparateRatioHeight(startRatio.y, endRatio.y);
    return pos;
}

// 右下を起点にした離れた割合(0～1)から座標を取得
Vector2_Int DisplaySize::RightDown_SeparateRatio(Vector2 startRatio, Vector2 endRatio)
{
    Vector2_Int pos;
    pos.x = Right_SeparateRatioWidth(startRatio.x, endRatio.x);
    pos.y = Down_SeparateRatioHeight(startRatio.y, endRatio.y);
